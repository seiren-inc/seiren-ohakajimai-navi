# Doc-03 Database Detail Design v1.1
...
## 11. Logical Deletion Policy

物理削除は禁止。全削除は論理削除フラグで管理する。

### 11.1 カラム定義

論理削除対象のエンティティには以下カラムを必須とする。

| カラム    | 型        | デフォルト | 説明       |
| --------- | --------- | ---------- | ---------- |
| isDeleted | boolean   | false      | 削除フラグ |
| deletedAt | timestamp | null       | 削除日時   |
| deletedBy | string    | null       | 削除実行者 |

### 11.2 削除時の動作

削除操作は以下の順序で実行する。

1. `isDeleted = true` にセット
2. `deletedAt` に実行日時を記録
3. `deletedBy` に実行者IDまたはシステム名を記録
4. 監査ログ（AuditLog）へ操作内容を保存

### 11.3 復元

- 論理削除されたレコードは `isDeleted = false` に戻すことで復元可能
- 復元操作も監査ログに記録する

### 11.4 禁止事項

- 物理削除（DELETE文）はDR（災害復旧）用途以外で禁止
- 論理削除フラグ未設定のエンティティへの物理削除操作は禁止
- 監査ログの論理削除・物理削除はいかなる状況でも禁止
...
| ------- | ----------------------------------------------------------- |
| v1.0    | 初版                                                        |
| v1.1    | Logical Deletion Policy（論理削除ポリシー）セクションを追加 |
