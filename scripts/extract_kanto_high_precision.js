...
/**
 * High Precision Extraction Script for Kanto Block (Native Node version)
 */
...
// Designated Cities and their Wards
...
for (const city in DESIGNATED_CITIES) {
    for (const ward of DESIGNATED_CITIES[city].wards) {
        ...
        WARD_TO_CITY[ward] = DESIGNATED_CITIES[city].cityJis;
    }
}

async function main() {
    console.log("Starting High-Precision Extraction (Node Native)...");
    ...
    console.log(`Found ${extractedLinks.length} total links.`);
    ...
    let newCount = 0;
    let updatedCount = 0;
    const matchedJisCodes = new Set();

    console.log("Matching links...");
    for (let i = 0; i < extractedLinks.length; i++) {
        ...
        } else if (WARD_TO_CITY[link.text]) {
        ...
        if (targetJis) {
            ...
            if (existingIdx >= 0) {
                ...
                let updated = false;
                if (isPdf) {
                    if (!current.pdfUrl || score > 0) { current.pdfUrl = link.href; updated = true; }
                } else {
                    if (!current.url || score > 0) { current.url = link.href; updated = true; }
                }
    ...
    const report = `# Kanto Block Extraction Report (Native)

## Summary
- **Total Kanto**: ${stats.totalKanto}
...
`;
    ...
    console.log("Done. Report at:", REPORT_PATH);
}
